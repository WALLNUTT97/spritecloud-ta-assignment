...
test.describe('DummyJSON API negative scenarios', () => {
  test('1. Attempt to perform a Login with invalid data', async ({ request }) => {
    const response = await request.post(`${apiConfig.dummyJsonBaseUrl}/auth/login`, {
      data: {
        username: 'notAUser',
  ...
  test('2. Get an AUTH token with no session', async ({ request }) => {
    const response = await request.get(`${apiConfig.dummyJsonBaseUrl}/auth/me`, {
      headers: {
