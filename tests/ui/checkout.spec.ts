...
test.describe("Full checkout flow from logged out state", () => {
  ...
  test("Complete checkout with two products and validates the final price", async ({
    page,
  }) => {
    ...
    await test.step("Validate basket contents", async () => {
      ...
      await cartPage.validateCartContents([
        SauceDemoProductInfo.backpack,
        SauceDemoProductInfo.bikeLight,
      ]);
    ...
    await test.step("Validate checkout totals", async () => {
      await checkoutPage.validatePriceTotal([
        SauceDemoProductInfo.backpack,
        SauceDemoProductInfo.bikeLight,
      ]);
    });
