...
import {
  ...
  SortDirections,
} from "../../src/data/sauceDemoProducts";
...
test.describe("SauceDemo product sorting", () => {
  ...
  test("Validate sorting functionality", async ({ page }) => {
    ...
    await test.step("Validate A-Z sorting", async () => {
      ...
      await inventoryPage.compareProductNameOrder(
        gatheredProductNames,
        SortDirections.nameAscending,
      );
    });
    ...
    await test.step("Validate Z-A sorting", async () => {
      ...
      await inventoryPage.compareProductNameOrder(
        gatheredProductNames,
        SortDirections.nameDescending,
      );
    });

    await test.step("Validate Price: Low to High sorting", async () => {
      await inventoryPage.sortBy(SortDirections.priceLowHigh);
      const gatheredProductPrices =
        await inventoryPage.getVisibleProductPrices();
      await inventoryPage.compareProductPriceOrder(
        gatheredProductPrices,
        SortDirections.priceLowHigh,
      );
    });
